  ...
  namespace :extensions do
    namespace :drag do
      ...
      task :update => :environment do
        ...
        unless DragExtension.root.starts_with? RAILS_ROOT # don't need to copy vendored tasks
          puts "Copying rake tasks from DragExtension"
          local_tasks_path = File.join(RAILS_ROOT, %w(lib tasks))
          mkdir_p local_tasks_path, :verbose => false
          Dir[File.join DragExtension.root, %w(lib tasks *.rake)].each do |file|
      ...
      task :sync => :environment do
        ...
        language_root = DragExtension.root + "/config/locales"
